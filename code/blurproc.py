...
# Function to determine the gaussian kernel size per depth level
# This function assigns the kernel size per depth level
# Input 'step' controls the gradient of the gaussian blurring around
...
# Function to create masks for each depth level for accurate blurring of frames
def newmaskimg(image, array, blurparam):
    ...
    imar = []
    # Generating the blurred image array
    for i in range(4):
...
# Combining images with same blur levels for faster computation
